...
53 tools across 14 modules — **50 enabled by default** (the 3 paid AI **Entities** tools are off unless you set `GONG_ENABLE_AI_ENTITIES=true`, since they consume Gong credits; `gong_entity_context` is the credit-free alternative):
...
| **Entities** | `gong_entity_context` — credit-free account/deal/contact/lead call context (the model answers from it). `gong_ask_account`, `gong_ask_deal`, `gong_generate_brief` — paid Gong AI Q&A/brief, **disabled by default** (consume credits) — see note below |
...
**`gong_entity_context`** — "What are the risks on the Acme deal?" "Brief me on this account." Gathers the recent calls for one CRM entity (account, deal, contact, or lead) into a single context block — outcomes, briefs, key points, next steps, topics, trackers, optional transcripts — and the model answers or writes the brief from it. **Credit-free** stand-in for the paid AI tools below: pass a Salesforce Account/Opportunity id for ACCOUNT/DEAL, or an email for CONTACT/LEAD.

> ⚠️ The next three tools (`gong_ask_account`, `gong_ask_deal`, `gong_generate_brief`) are **disabled by default** — they consume paid Gong credits. See the note below to enable, or use `gong_entity_context` above.
